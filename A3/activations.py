...
class Activation:
    ...
    def forward_relu(self, Z):
        ...
        f(z): as described above applied elementwise to `Z`
        """
        return np.maximum(0, Z)  # element-wise max
    ...
    def forward_softmax(self, Z):
        ...
        Z_exp = np.exp(Z - m) # subtract m to ensure numerical stability
        return Z_exp / np.sum(Z_exp, axis=1, keepdims=True) # normalize
