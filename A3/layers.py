...
class FullyConnectedLayer():
    ...
    def forward(self, X):
        ...
        Z = X.dot(self.parameters["W"]) + self.parameters["b"] # Z = XW + transposed(1)b
    ...
    def backward(self, dLdY):
        ...
        dLdZ = self.activation.backward(Z, dLdY) # gradient of loss w.r.t Z

        dW = (X.T).dot(dLdZ) # gradient of los w.r.t W
        db = np.sum(dLdZ, axis=0, keepdims=True) # gradient of loss w.r.t b
        dX = dLdZ.dot(self.parameters["W"].T) # gradient of loss w.r.t X
