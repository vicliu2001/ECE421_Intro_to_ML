...
class VanillaRNN(nn.Module):
  def __init__(self,
    ...
    self.rnn = nn.RNN(input_size, hidden_size, num_layers=1, nonlinearity=nonlinearity, batch_first=True)
  ...
  def forward(self, x):
    ...
    out = self.fc(self.rnn(x, h0)[0][:, -1, :].unsqueeze(1))
...
class SimpleLSTM(nn.Module):
  ...
  def forward(self, x):
    ...
    out = self.fc(self.lstm(x, (h0, c0))[0][:, -1, :].unsqueeze(1))
