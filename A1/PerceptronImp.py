...
def fit_perceptron(X_train, y_train, max_epochs=5000):
    ...
    "*** YOUR CODE HERE ***"
    X_aug = np.c_[np.ones(row), X_train]
    ...
    for i in range(max_epochs):
        for j in range(row):
          ...
          if pred(X_aug[j], w) != y_train[j]:
            misclassified_point = X_aug[j] * (y_train[j])
          
            w += misclassified_point
            best_w = w
            Ein_best = errorPer(X_aug, y_train, w)
            break
...
def errorPer(X_train, y_train, w):
    ...
    predicted_y = []
    for x in X_train:
      predicted_y.append(pred(x, w))

    err = 0
    for i in range(len(predicted_y)):
      if predicted_y[i] != y_train[i]:
        err += 1
    
    avgError = err/len(predicted_y)
    return avgError
...
def confMatrix(X_train, y_train, w):
    ...
    for i in range(len(X_aug)):
      if pred(X_aug[i], w) == -1 and y_train[i] == -1:
        conf_mat[0, 0] += 1
      elif pred(X_aug[i], w) == 1 and y_train[i] == -1:
        conf_mat[0, 1] += 1
      elif pred(X_aug[i], w) == -1 and y_train[i] == 1:
        conf_mat[1, 0] += 1
      else:
